...
const COMMUNITY_URL = "https://chat.whatsapp.com/FbFih6XGSd18Ov0emoeEba";
...
function SuccessContent() {
  ...
  // If the member opted into WhatsApp groups during registration, count down
  // and send them straight to the community instead of making them tap.
  const autoJoin = searchParams.get("wa") === "1";
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      const left = Math.max(0, REDIRECT_MS - (Date.now() - started));
      setRemaining(left);
      if (left === 0) {
        clearInterval(tick);
        setRedirecting(true);
  ...
  const progress = (remaining / REDIRECT_MS) * 100;
